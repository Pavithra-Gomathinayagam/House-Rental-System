...
public class OwnerDAO {
	...
	public int addRentalDetails(Rental rental)throws SQLException
	{
		try {
			...
			if(resultSet.next())
			{
				//if(resultSet.getString(2).equalsIgnoreCase(rental.getRentarea())&&resultSet.getString(3).equals(String.valueOf(rental.getZipcode())))
				String id = resultSet.getString(1);
			...
			System.out.println(rental.getRentaddress());
			...
			if(resultSet.next())
				if(resultSet.getString(1).equals(String.valueOf(areaId)))
				{
					...
					System.out.println(addressId+"p");
				}
			...
			prepareStatement.setString(7, rental.getLandmark());
			//prepareStatement.setString(7, formatDateTime);
			prepareStatement.executeUpdate();
	...
	public void addRentDetails(Rental rental)throws SQLException
	{
		try {
			...
			prepareStatement = connection.prepareStatement("SELECT FNAME FROM USER WHERE USER_ID = ?");
			prepareStatement.setInt(1, userId);
			resultSet = prepareStatement.executeQuery();
			if(resultSet.next())
	...
	public void addFacility(Rental rental)throws SQLException
	{
		try {
			...
			if(resultSet.next())
			{//if(resultSet.getString(2).equals(String.valueOf(userId)))		
					houseId = resultSet.getInt(1);		
			}
			System.out.println(houseId);
	...
	public int updateRentAddress(Rental rental)throws SQLException
	{
		...
		try {
			...
			*/	prepareStatement = connection.prepareStatement(SqlConstant.RETRIEVE_AREA_ID);
				...
				prepareStatement = connection.prepareStatement("SELECT ADDRESS_ID FROM RENTAL_DETAILS WHERE HOUSE_ID = ?");
	...
	public ArrayList<Rental> viewHouse(Rental rental)throws SQLException
	{
		...
		try {
			...
			while(resultSet.next())
			{
				...
				houseId = resultSet.getInt(1);
				prepareStatement1 = connection.prepareStatement("SELECT HOUSE_ID FROM USER_RENTAL_INTEREST WHERE USER_RENTAL_INTEREST.HOUSE_ID = ?");
...
//				System.out.println(resultSet.getInt(1));
	...
	public ArrayList<User> viewInterestedTenant(User user)throws SQLException
	{
		...
		try {
			...
			while(resultSet.next())
			{
				...
				if(resultSet1.next())
				{
					//logger.log(Level.INFO, "\nUSER ID:"+resultSet1.getInt(1)+"\nFNAME:"+resultSet1.getString(2)+"\nEMAIL:"+resultSet1.getString(3)+"\nPHONE NO:"+resultSet1.getLong(5)+"\nOCCUPATION:"+resultSet1.getString(6));
				...
				if(resultSet2.next())
				{
					//logger.log(Level.INFO, "\nADDRESS:"+resultSet2.getString(2));
				...
				if(resultSet3.next())
				{
					//logger.log(Level.INFO, "\nAREA:"+resultSet3.getString(2)+"\nZIPCODE:"+resultSet3.getString(3));
	...
	public int checkAcceptedRequest(Rental rental)throws SQLException
	{
		try {
			...
			int houseId = 0;
			int flag = 0;
			prepareStatement = connection.prepareStatement("SELECT USER_RENTAL_INTEREST.USER_ID,USER_RENTAL_INTEREST.HOUSE_ID FROM USER_RENTAL_INTEREST,USER WHERE USER.EMAIL = ? AND USER.USER_ID = USER_RENTAL_INTEREST.USER_ID AND USER_RENTAL_INTEREST.HOUSE_ID = ? AND USER_RENTAL_INTEREST.STATUS_ID = 2");
			prepareStatement.setString(1, rental.getEmail());
			prepareStatement.setInt(2, rental.getHouseid());
			resultSet = prepareStatement.executeQuery();
			if(resultSet.next())
	...
	public void acceptUserRequest(Rental rental)throws SQLException
	{
		try {
			...
			prepareStatement = connection.prepareStatement(SqlConstant.UPDATE_STATUS);
			prepareStatement.setInt(1, 3);
			prepareStatement.setInt(2, rental.getUserid());
			prepareStatement.setInt(3, houseId);
			prepareStatement.executeUpdate();
			//logger.log(Level.INFO, "Number of rows affected: " + cnt);
			*/
		}catch(SQLException e)
	...
	public ArrayList<Rental> viewStatus(Rental rental)throws SQLException
	{
		...
		try {
			...
			while(resultSet.next())
			{
				...
				/*addressId = resultSet1.getInt(2);
				statusId = resultSet1.getInt(3);
				
				System.out.println(addressId);
				prepareStatement2 = connection.prepareStatement(SqlConstant.RETRIEVE_ADDRESS_AREA);
				prepareStatement2.setInt(1, addressId);
				resultSet2 = prepareStatement2.executeQuery();
				if(resultSet2.next())
				{
					//logger.log(Level.INFO, "\nADDRESS:"+resultSet2.getString(1));
					rent.setRentaddress(resultSet2.getString(1));
					areaId = resultSet2.getInt(2);
					System.out.println(areaId);
				}
				prepareStatement3 = connection.prepareStatement(SqlConstant.RETRIEVE_AREA);;
				...
				{
					//logger.log(Level.INFO, "\nAREA:"+resultSet3.getString(2)+"\nZIPCODE:"+resultSet3.getString(3));
					rent.setRentarea(resultSet3.getString(2));
					rent.setZipcode(resultSet3.getInt(3));
				}
				prepareStatement = connection.prepareStatement(SqlConstant.SELECT_STATUS_ID);
				prepareStatement.setInt(1, statusId);
				resultSet = prepareStatement.executeQuery();
				if(resultSet.next())
				{
					//logger.log(Level.INFO, "\nSTATUS:"+resultSet3.getString(2));
					rent.setStatus(resultSet.getString(1));
	...
	public ArrayList<Rental> viewConformedCustomer(Rental rental)throws SQLException
	{
		try {
			...
			connection = DBUtils.getConnection();
			prepareStatement = connection.prepareStatement("SELECT HOUSE_ID FROM USER,RENTAL_DETAILS WHERE USER.EMAIL = ? AND USER.USER_ID = RENTAL_DETAILS.USER_ID");
			prepareStatement.setString(1, rental.getEmail());
			resultSet = prepareStatement.executeQuery();
			int houseId = 0;
			int userId = 0;
			while(resultSet.next())
			{
				...
				prepareStatement2 = connection.prepareStatement("SELECT HOUSE_ID FROM USER_RENTAL_INTEREST WHERE HOUSE_ID = ? AND STATUS_ID = 4");
				prepareStatement2.setInt(1, houseId);
				resultSet2 = prepareStatement2.executeQuery();
				if(resultSet2.next())
				{
				...
				Rental rent = new Rental();
				prepareStatement1 = connection.prepareStatement("SELECT DISTINCT ADDRESS,AREA,RENTAL_CHOICE,PRICE,STATUS FROM USER_RENTAL_INTEREST,ADDRESS,RENTAL_DETAILS,AREA,RENTAL_STATUS,RENTAL_OPTION WHERE RENTAL_DETAILS.HOUSE_ID = ? AND RENTAL_DETAILS.ADDRESS_ID = ADDRESS.ADDRESS_ID AND ADDRESS.AREA_ID = AREA.AREA_ID AND RENTAL_DETAILS.CHOICE_ID = RENTAL_OPTION.CHOICE_ID AND RENTAL_DETAILS.HOUSE_ID = USER_RENTAL_INTEREST.HOUSE_ID AND RENTAL_STATUS.STATUS_ID = USER_RENTAL_INTEREST.STATUS_ID AND USER_RENTAL_INTEREST.STATUS_ID = 4");
				prepareStatement1.setInt(1, houseId);
				resultSet1 = prepareStatement1.executeQuery();
	...
	public ArrayList<Rental> viewRejected(Rental rental)throws SQLException
	{
		try {
			...
			while(resultSet.next())
			{
				...
				prepareStatement1 = connection.prepareStatement("SELECT DISTINCT ADDRESS,AREA,RENTAL_CHOICE,PRICE,STATUS FROM USER_RENTAL_INTEREST,ADDRESS,RENTAL_DETAILS,AREA,RENTAL_STATUS,RENTAL_OPTION WHERE RENTAL_DETAILS.HOUSE_ID = ? AND RENTAL_DETAILS.ADDRESS_ID = ADDRESS.ADDRESS_ID AND ADDRESS.AREA_ID = AREA.AREA_ID AND RENTAL_DETAILS.CHOICE_ID = RENTAL_OPTION.CHOICE_ID AND RENTAL_DETAILS.HOUSE_ID = USER_RENTAL_INTEREST.HOUSE_ID AND RENTAL_STATUS.STATUS_ID = USER_RENTAL_INTEREST.STATUS_ID AND USER_RENTAL_INTEREST.STATUS_ID = 3");
